...
// CARTEIRA X1 (rota /x1/carteira): a tela antiga inteira ficou aqui só pela
// carteira (depósito Pix/MP, saque, histórico). A arena nova mora em X1.tsx.
export default function X1Carteira() {
  const { user } = useAuth();
  ...
  const [walletOpen, setWalletOpen] = useState(true); // abre a "tela" da carteira (saque/depósito)
    ...
    <div className="pb-24 px-4 pt-4 max-w-2xl mx-auto space-y-4">
      ...
      <div className="fixed inset-0 z-[60] bg-background overflow-y-auto">
        <div className="max-w-2xl mx-auto px-4 pt-4 pb-24 space-y-3">
          <div className="flex items-center gap-3">
            <button onClick={() => { setWalletOpen(false); setWithdrawOpen(false); setDepositOpen(false); navigate("/x1"); }} className="w-9 h-9 rounded-full flex items-center justify-center text-muted-foreground hover:bg-muted/40">
              <ArrowLeft className="w-5 h-5" />
            </button>
