/* ============================================================
   ...
   aprovou no mockup "mock-final3" + "mock-fluxo":
     ...
     1. RECEBIMENTOS  "Do que você vendeu, quanto entrou?"
                      dinheiro / pix / cartão → o CALOTE é calculado, nunca digitado
     2. CUSTOS        "Os custos que eu já sei são estes. Teve mais algum?"
                      mercadoria (CMV) automática, transporte/comida em zero,
                      tudo editável, "+ adicionar outro custo". Zero não conta.
     3. RELATÓRIO     premium: sobrou pra você, vendido/recebido/fiado,
                      como o dinheiro entrou, o seu dia, melhor hora, compartilhar
     4. O QUE MEXEU   ranking, calote, constância, amanhã → FECHAR O DIA
...
import {
  ...
  Check, Clock, Trophy, Coins, Flame, ChevronRight, Loader2, Share2, RotateCcw, ArrowLeft,
} from "lucide-react";
...
type Passo = "recebimentos" | "custos" | "relatorio" | "mexeu";
...
function Passos({ n }: { n: 1 | 2 | 3 }) {
  return (
    <>
      <p className="orbis-mini">Fechamento · passo {n} de 3</p>
      <div className="flex gap-1.5 mt-3">
        {[1, 2, 3].map((k) => <span key={k} className="h-[3px] flex-1 rounded-full" style={{ background: k <= n ? "var(--orbis-gold)" : "rgba(255,255,255,.10)" }} />)}
      </div>
    </>
  );
}
...
export function DefconFechamento({
  ...
  const [passo, setPasso] = useState<Passo>("recebimentos");
  ...
  /* ============ 1 · RECEBIMENTOS ============ */
  if (passo === "recebimentos") {
    const campos: [keyof typeof rec, string, ReactNode][] = [
      ["dinheiro", "Dinheiro", <Banknote key="d" className="w-[17px] h-[17px]" strokeWidth={2.1} style={{ color: "var(--orbis-ok)" }} />],
      ["pix", "Pix", <Smartphone key="p" className="w-[17px] h-[17px]" strokeWidth={2.1} style={{ color: "var(--orbis-gold)" }} />],
      ["cartao", "Cartão", <CreditCard key="c" className="w-[17px] h-[17px]" strokeWidth={2.1} style={{ color: "var(--orbis-fg-2)" }} />],
    ];
    return (
      <div className="min-h-[100dvh] bg-background pt-safe pb-safe px-5 pt-4 pb-10 max-w-md mx-auto orbis-stagger">
        <Passos n={1} />
        <h1 className="font-display text-[22px] font-extrabold leading-tight mt-4">Do que você vendeu,<br />quanto entrou?</h1>
        <p className="text-[12.5px] mt-2" style={{ color: "var(--orbis-fg-3)" }}>Vendeu <b style={{ color: "var(--orbis-fg)" }}>{formatCurrency(vendidoSemGorjeta)}</b>{gorjetas > 0 && <> · mais {formatCurrency(gorjetas)} de gorjeta</>}</p>
        ...
        <Bloco style={{ marginTop: 18 }}>
          {campos.map(([k, rot, ico], idx) => (
            <div key={k} className="flex items-center gap-3 px-4 h-[62px]" style={idx ? { borderTop: "1px solid var(--orbis-line)" } : undefined}>
              ...
              <CampoValor valor={rec[k]} onChange={(v) => setRec({ ...rec, [k]: v })} destaque={!!rec[k]} />
            </div>
          ))}
        </Bloco>

        {fiado > 0 && vendidoSemGorjeta > 0 && (
          <div className="rounded-[16px] border mt-3 p-3.5 flex items-start gap-3" style={{ borderColor: "rgba(229,115,127,.32)", background: "rgba(229,115,127,.08)" }}>
            <span className="w-7 h-7 rounded-[9px] flex items-center justify-center shrink-0" style={{ background: "rgba(229,115,127,.16)" }}><AlertTriangle className="w-4 h-4" style={{ color: "var(--orbis-custo)" }} strokeWidth={2.4} /></span>
            <span className="flex-1 min-w-0">
              <b className="block text-[14px] font-semibold" style={{ color: "var(--orbis-custo)" }}>{formatCurrency(fiado)} ficaram de pagar</b>
              <small className="block text-[12px] mt-0.5" style={{ color: "var(--orbis-fg-3)" }}>Vendeu {formatCurrency(vendidoSemGorjeta)} · recebeu {formatCurrency(recebido)}</small>
            </span>
          </div>
        )}
        {recebido > vendidoSemGorjeta + 0.005 && (
          <p className="text-[12px] mt-3 px-1" style={{ color: "var(--orbis-custo)" }}>Entrou mais do que você vendeu — confere os valores.</p>
        )}

        <button onClick={salvarRecebimentos} disabled={salvandoRec} className="orbis-cta w-full mt-5">
          {salvandoRec ? <Loader2 className="w-5 h-5 animate-spin" /> : "CONTINUAR"}
        </button>
        ...
        <div className="mt-6 flex flex-col items-center gap-1">
          {onExtend && (
            <button onClick={reabrir} disabled={reabrindo} className="h-10 text-[13px] font-semibold inline-flex items-center gap-1.5" style={{ color: "var(--orbis-fg-3)" }}>
              <ArrowLeft className="w-3.5 h-3.5" /> voltar e vender mais
            </button>
          )}
          {onRestart && !confirmarDescartar && (
            <button onClick={() => setConfirmarDescartar(true)} className="h-9 text-[12px]" style={{ color: "var(--orbis-fg-3)" }}>descartar este dia</button>
          )}
          {onRestart && confirmarDescartar && (
            <div className="rounded-[14px] border p-3 w-full text-center" style={{ borderColor: "rgba(229,115,127,.32)" }}>
              ...
              <div className="flex gap-2 mt-2">
                <button onClick={() => { void onRestart(); }} className="flex-1 h-9 rounded-[10px] text-[12.5px] font-bold" style={{ background: "rgba(229,115,127,.15)", color: "var(--orbis-custo)" }}><RotateCcw className="w-3.5 h-3.5 inline mr-1" />Sim, descartar</button>
  ...
  if (passo === "custos") {
    return (
      <div className="min-h-[100dvh] bg-background pt-safe pb-safe px-5 pt-4 pb-10 max-w-md mx-auto orbis-stagger">
        ...
        <button onClick={() => setPasso("recebimentos")} className="w-full h-10 mt-1 text-[13px] font-semibold inline-flex items-center justify-center gap-1.5" style={{ color: "var(--orbis-fg-3)" }}><ArrowLeft className="w-3.5 h-3.5" /> voltar</button>
      </div>
    );
  }
  ...
  if (passo === "relatorio") {
    ...
    return (
      <div className="min-h-[100dvh] bg-background pt-safe pb-safe px-5 pt-4 pb-10 max-w-md mx-auto orbis-stagger">
        ...
        <button onClick={() => setMostrarShare((v) => !v)} className="w-full h-[46px] rounded-[14px] mt-2.5 text-[13.5px] font-semibold inline-flex items-center justify-center gap-2" style={{ border: "1px solid rgba(255,255,255,.12)", color: "var(--orbis-fg-2)" }}>
          <Share2 className="w-4 h-4" /> {mostrarShare ? "Fechar artes" : "Compartilhar resultado"}
        </button>
        {mostrarShare && (
          <div className="mt-3">
    ...
    <div className="min-h-[100dvh] bg-background pt-safe pb-safe px-5 pt-4 pb-10 max-w-md mx-auto orbis-stagger">
      ...
      {onExtend && (
        <button onClick={reabrir} disabled={reabrindo} className="w-full h-[46px] rounded-[14px] mt-2.5 text-[13.5px] font-semibold inline-flex items-center justify-center gap-2" style={{ border: "1px solid rgba(255,255,255,.12)", color: "var(--orbis-fg-2)" }}>
          {reabrindo ? <Loader2 className="w-4 h-4 animate-spin" /> : <>Reabrir e vender mais <ChevronRight className="w-4 h-4" /></>}
        </button>
      )}
