...
// Paleta ORIGINAL colorida: Lenda roxo, Grao-Mestre branco-gelo, Mestre/Diamante ciano,
// Ouro dourado, Platina prata-azulada, Bronze cobre — cada liga com sua cor e escudo.
export function getTier(pos: number): Tier {
  ...
  if (pos <= 20) return { color: "#F2B43A", glow: "rgba(242,180,58,0.5)", label: "OURO", rank: 3, icon: ouroIcon };
  if (pos <= 45) return { color: "#9FB2CC", glow: "rgba(159,178,204,0.45)", label: "PLATINA", rank: 2, icon: platinaIcon };
