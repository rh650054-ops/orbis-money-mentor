-- Orbis — pontuação da competição pelo EXTRATO VERIFICADO (nosso sistema de IA).
-- Antes: pix_revenue somava hourly_goal_blocks.valor_pix (automático, sem conferência).
-- Agora: soma extrato_uploads.total_verificado (card+pix CONFERIDO pela IA) no período.
-- Dinheiro vivo NÃO entra (a IA já exclui). Dia sem extrato = 0 naquele dia.
...
BEGIN
  ...
  IF comp.metric = 'pix_revenue' THEN
    -- Faturamento VERIFICADO (card+pix conferido no extrato)
    UPDATE public.competition_participants cp
    SET score = COALESCE(sub.total, 0), updated_at = now()
    FROM (
      SELECT user_id, SUM(COALESCE(total_verificado, 0)) AS total
  ...
  ELSIF comp.metric = 'pix_sales_count' THEN
    -- Nº de vendas CONFERIDAS no extrato
    UPDATE public.competition_participants cp
...
-- Recalcula AUTOMÁTICO quando o vendedor sobe/atualiza o extrato do dia:
-- mexe só nas competições ATIVAS dele que incluem aquele dia.
...
BEGIN
  FOR c IN
    ...
    WHERE cp.user_id = NEW.user_id
      ...
      AND NEW.dia >= comp.starts_at::date
      AND NEW.dia <= comp.ends_at::date
  LOOP
