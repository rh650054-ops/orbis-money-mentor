...
(function () {
  ...
  function nivelHTML() {
    ...
    return '<div class="nivel">' +
      ...
      '<p class="nota">1 cliente ativo no plano Mensal = 1 VP. Cliente que cancela ou deixa de pagar deixa de contar.</p></div>';
  }
  ...
  function renderConquistas() {
    ...
    $("p-conquistas").innerHTML =
      ...
      det("Níveis e VP (Vant Points)", "VP contam só assinaturas ativas: " + (r.vp_planos || []).map(function (p) { return p.nome + " (" + brl(p.preco) + ") = " + p.vp + " VP"; }).join(" · ") + ". Cliente que cancela ou deixa de pagar deixa de contar. Níveis: " + (D.nivel.niveis || []).map(function (n) { return n.nome + " a partir de " + n.vp_min + " VP"; }).join(", ") + ". " + (D.nivel.progressao_ativa ? "O nível altera o percentual recorrente." : "Por enquanto os níveis são reconhecimento e não alteram a comissão.")) +
